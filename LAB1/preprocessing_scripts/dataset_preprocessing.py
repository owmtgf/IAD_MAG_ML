...
def fill_not_reported_field(
    ...
    df_out[feature] = df_out[feature].apply(
        lambda v: np.nan if (pd.isna(v) or v in MISSING_TOKENS) else v
    )
...
def split_feature_components(
    df: pd.DataFrame,
    feature: str = "Primary_Diagnosis"
) -> pd.DataFrame:
    ...
    unique_values = df_out[feature].dropna().unique()
    ...
    for term in terms:
        col_name = f"{feature}_{term.replace(' ', '_')}"
...
def convert_lifetime(df: pd.DataFrame, feature: str = "Age_at_diagnosis") -> pd.DataFrame:
    ...
    def parse_age(value):
        if pd.isna(value) or value in MISSING_TOKENS:
            return np.nan
